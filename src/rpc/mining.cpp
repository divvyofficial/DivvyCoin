...
UniValue generatetoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
            "2. address    (string, required) The address to send the newly generated divvycoin to.\n"
...
//            "  \"masternode_payments_enforced\" : true|false, (boolean) true, if masternode payments are enforced\n"
    ...
    if (params.size() > 0)
    {
        ...
        if (strMode == "proposal")
        {
            ...
            {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "DivvyCoin is processing blocks...");
            }
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DivvyCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DivvyCoin is downloading blocks...");

//  Enable after smartnodes are active
//    if (!smartnodeSync.IsSynced())
//         throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DivvyCoin is syncing with network...");
